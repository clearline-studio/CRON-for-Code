...
describe('chat runtime routing (cloud-first, Ollama fallback)', () => {
  it('defaults to a cloud provider with an Ollama local fallback', () => {
    ...
    expect(DEFAULT_LLM_CONFIG.cloud.codingModel).toBe('opencode-go/deepseek-v4-flash');
    expect(DEFAULT_LLM_CONFIG.cloud.codingVisionModel).toBe('opencode-go/deepseek-v4-flash-vision-exp');
  ...
  it('builds governed OpenCode handoff tasks with the configured cloud models', () => {
    ...
    expect(handoff.body).toContain('Primary coding model: opencode-go/deepseek-v4-flash');
