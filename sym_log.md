...
## 2026-08-24 (late) - Gem P — "lock the canvas" layout fix (her #1 item)

**Decision (Venessa chose):** panels float OVER the chat — the centre never moves or resizes when the left Projects list or a right panel opens/closes.

**Current cause of the jump:** `Layout.tsx` renders the Projects panel inline in the left column and `RightSidebar.tsx` renders the open panel inline in the right edge, both in the flex flow → the centre (`flex: 1`) shrinks when either opens.
